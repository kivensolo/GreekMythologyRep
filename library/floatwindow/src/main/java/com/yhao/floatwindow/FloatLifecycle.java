...
class FloatLifecycle extends BroadcastReceiver implements Application.ActivityLifecycleCallbacks {
    ...
    public void onActivityResumed(Activity activity) {
        ...
        if (needShow(activity)) {
            mLifecycleListener.onResumeShow();
        } else {
    ...
    public void onActivityStarted(Activity activity) {
        Log.d("onActivity","onActivityStarted :" +activity.getComponentName().getShortClassName());
        startCount++;
    }
    ...
    public void onActivityStopped(Activity activity) {
        Log.d("onActivity","onActivityStopped :" +activity.getComponentName().getShortClassName());
         startCount--;
         //FIXME 暂时屏蔽，异常回调两次
        if (startCount == 0) {
            //mLifecycleListener.onBackToDesktop();
