package com.kingz.net.retrofit.mannager;
...
//    }
    ...
    public void registServer(Class clazz) {
...
//    }
     ...
     * 使用RxJava设置数据订阅对象
     *
     * @param observable 数据观察对象<T>
     * @param observer   数据观察者
     * @param <T>        T
     */
    public <T> void setSubscribe(Observable<T> observable,
                                 Observer<T> observer) {
        observable.subscribeOn(Schedulers.io())           // 代表IO线程加载数据 通常用于网络,读写文件等io密集型的操作
